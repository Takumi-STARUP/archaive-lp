  ...
  return (
    <section className="py-20 bg-black text-white shadow-2xl rounded-3xl">
      <div className="container mx-auto px-4 max-w-5xl text-center">
        <h2 className="text-4xl font-extrabold mb-10 text-white text-center drop-shadow">ご利用開始までのかんたん3ステップ</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {steps.map((step, idx) => (
            <div key={idx} className="flex flex-col items-center bg-[#37B7C4]/10 rounded-2xl shadow-lg p-10 border-2 border-[#37B7C4]/20">
              <Icon type={step.icon as IconType} size={48} className="mb-4" />
              <h3 className="text-2xl font-bold text-[#37B7C4] mb-2">{step.title}</h3>
              <p className="text-white text-center text-lg">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
