...
export default function ProblemSection8() {
  return (
    <section id="problem8" className="py-16 bg-[#FAFAF8] text-gray-700 scroll-mt-20">
      <div className="container mx-auto px-4 max-w-6xl">
        {/* セクションヘッダー */}
        <div className="text-center mb-16">
          <div className="inline-block bg-[#8B3A3A] px-6 py-2 mb-6">
            <span className="text-gray-700 font-medium tracking-wider">PROBLEMS</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-medium text-gray-700 mb-4 leading-tight">
            「人の経験」に頼った見積もりは、
            <span className="block text-[#8B3A3A]">もう限界です。</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {problems.map((problem, idx) => (
            <div key={idx} className="group">
              <div className="bg-white p-8 h-full border-l-4 border-[#8B3A3A] relative overflow-hidden hover:shadow-2xl transition-all duration-300">
                {/* 背景装飾 */}
                ...
                <div className="relative z-10">
                  {/* アイコン */}
                  <div className="mb-4">
                    <Icon type={problem.icon as IconType} size={48} className="text-black group-hover:text-[#8B3A3A] transition-colors duration-300" />
                  </div>

                  {/* タイトル */}
                  <h3 className="font-medium text-xl text-black mb-3 leading-tight">
                    {problem.title}
                  </h3>
                  ...
                  {/* 装飾ライン */}
                  <div className="mt-6">
                    <div className="w-12 h-0.5 bg-[#8B3A3A] group-hover:w-20 transition-all duration-300"></div>
                  </div>
                </div>
              </div>
            </div>
